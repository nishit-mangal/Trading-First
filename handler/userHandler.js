...
export async function findValidOTPsAndCheckIfValid(userId, otp, type) {
	...
	if (!validOTP) return { status: "Err", msg: "No valid OTP found." };

	if (validOTP.otp !== otp) return { status: "Err", msg: "Invalid OTP" };
...
export async function saveOTPinDB(otp, userId, type) {
	try {
		if (!otp || !userId || !type) throw "No OTP received. Not saving to DB";
...
export async function setPinProcess(user, otp, pin) {
	...
	try {
		let validOTP = await findValidOTPsAndCheckIfValid(
			user.id,
			otp,
