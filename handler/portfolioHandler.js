...
export async function sortHoldingData(holdingArr) {
	...
	for (let stock of holdingArr) {
		...
		if (isDataInCache == 1) {
			console.log(
				`Cache Hit for ${CACHE_NAMES.STOCK.NAME}:${stock.company_name}`
			);
		...
		// await redisClient.lpush(CACHE_NAMES.PORTFOLIO_HOLDINGS.NAME, JSON.stringify(tempStockObj), ()=>console.log(`${CACHE_NAMES.PORTFOLIO_HOLDINGS.NAME} details set in Cache`))
	}
	// await redisClient.expire(CACHE_NAMES.PORTFOLIO_HOLDINGS.NAME, CACHE_NAMES.PORTFOLIO_HOLDINGS.TTL)
	return filteredArr;
}
