...
import {
	...
	createUserInDB,
	findValidOTPsAndCheckIfValid,
	getUserById,
	getUserWithEmailAndPin,
	markUserAccountVerified,
	saveOTPinDB,
	sendEmailVerifiationCode,
	setAPISecret,
	setPinProcess,
...
export async function getFundDetails(req, res) {
	try {
		...
		let isDataInCache = await redisClient.exists(CACHE_NAMES.FUND_DETAILS.NAME);
		if (isDataInCache == 1) {
			console.log(`Cache Hit for ${CACHE_NAMES.FUND_DETAILS.NAME}`);
			let cacheData = await redisClient.get(CACHE_NAMES.FUND_DETAILS.NAME);
		...
		await redisClient.setex(
			CACHE_NAMES.FUND_DETAILS.NAME,
			CACHE_NAMES.FUND_DETAILS.TTL,
			JSON.stringify(responseObj),
			() => console.log("Fund Details set in Cache")
		);
	...
	try {
		...
		if (existingUserResp)
			throw {
				...
				msg: "User already exists with the entered email."
			};
		...
		if (!user)
			throw {
				...
				msg: "Failure in creating user"
			};

		let otpResponse = sendEmailVerifiationCode(
			user.data.email,
			OTP_TYPES.EMAIL_VERIFICATION.NAME
		);
...
export async function resetPassword(req, resp) {
	...
	try {
		...
		if (
			...
			!reqObj.currPassword ||
			!reqObj.newPassword ||
			!reqObj.confirmNewPassword ||
			reqObj.newPassword !== reqObj.confirmNewPassword
		) {
			response.responseCode = HttpCode.BAD_REQUEST;
			response.responseMessage = HTTP_MESSAGE.INVALID_INPUT;
		...
		let updateResp = await updatePasswordWithEmail(
			reqObj.email,
			hashedPassword
		);
		if (!updateResp) throw "Password update failed";
