...
export async function getHoldings(req, res) {
	try {
		...
		let isDataInCache = await redisClient.exists(
		...
		if (isDataInCache == 1) {
			...
			if (portfolioHoldings)
				await redisClient.setex(
					`${CACHE_NAMES.PORTFOLIO_HOLDINGS.NAME}`,
					CACHE_NAMES.PORTFOLIO_HOLDINGS.TTL,
					JSON.stringify(portfolioHoldings),
					() =>
						console.log(
							`${CACHE_NAMES.PORTFOLIO_HOLDINGS.NAME} Details set in Cache`
						)
				);
		}
		...
		for (let stock of filteredHoldings) {
			...
			let percentage = Math.floor(
				((stock.closingPrice - stock.oneYearMax) / stock.oneYearMax) * 100
			);
			...
			await redisClient.setex(
				`${CACHE_NAMES.STOCK.NAME}:${stock.companyName}`,
				CACHE_NAMES.STOCK.TTL,
				JSON.stringify(stock),
				() =>
					console.log(
						`${CACHE_NAMES.STOCK.NAME}:${stock.companyName} Details set in Cache`
					)
			);
		}

		filteredHoldings.sort((a, b) => {
			return a.percentFromMax - b.percentFromMax;
		});
		return res.json({
			status: "Success",
			statusCode: "200",
